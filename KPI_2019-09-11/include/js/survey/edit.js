...
$(function () {
    ...
    $('input:checkbox[name=end_check_1]').on('change', function() {
        ...
        if ($(this).is(':checked')) {
            ...
            end_check_flag = 1;
            $('#end_comment_1').val("설문참여에 감사드립니다.");
...
// 신규작성버튼클릭
function create_new_survey(survey_flag)
{    
    location.href = site_url + 'survey/view?survey_flag=' + survey_flag;
}

// 불러오기버튼클릭
function browse_draft_survey(survey_flag)
{        
    if (survey_flag == 1)
        location.href = site_url + 'survey/load_list/public';
    else
        location.href = site_url + 'survey/load_list/advanced';
}
...
function post()
{
    ...
    var survey_title = $('#education_title').val();
    var education_id = $('#education_id').val();

    //교육과정,교육차수,고객사,강사이름저장
    var education_course = $("#education_caption").val();
    var education_count = $("#education_count").val();
    var education_customer = $("#education_customer").val();
    var education_teacher = $("#education_teacher_1").val()+','+
                            $("#education_teacher_2").val() + ','
                            $("#education_teacher_3").val() + ','
                            $("#education_teacher_4").val() + ','
                            $("#education_teacher_5").val();

    if (survey_title == '') {
        valid = false;
        swal({title: '', text: '설문제목을 입력하십시요.',
                confirmButtonText: '확인', allowOutsideClick: false, type: 'warning'},
            function(isConfirm) {
                $('#education_title').focus();
            });
        return;
    }
    ...
    params.survey_flag = survey_flag;
    ...
    params.education_count = education_count;
    params.education_customer = education_customer;
    params.education_course = education_course;
    params.education_teacher = education_teacher;
    ...
    $('.survey-group-container').each(function(question_group_index) {
        ...
        $(this).find('.survey-question-container').each(function (question_index) {
            ...
            if (question.type == 0) {
                ...
                question.reverse_question = $(this).find('.reverse_question_1').is(':checked') ? 1 : 0;
                //문항이동값배열얻기
                var move_value_list = "";
                if(question.example_count == 5) {
                    $(this).find('.type-grade-inputs-5 .question-move').each(function (index) {
                        move_value_list += $(this).val()+",";
                    });
                }else if(question.example_count == 3){
                    $(this).find('.type-grade-inputs-3 .question-move').each(function (index) {
                        move_value_list += $(this).val()+",";
                    });
                }
                move_value_list = move_value_list.substring(0,move_value_list.length - 1);
                question.rating_move_value = move_value_list;
                
                //만족도항목이름얻기
                var rating_names = "";
                if(question.example_count == 5) {
                    $(this).find('.type-grade-inputs-5 .rating-name').each(function (index) {
                        rating_names += $(this).val()+",";
                    });
                }else if(question.example_count == 3){
                    $(this).find('.type-grade-inputs-3 .rating-name').each(function (index) {
                        rating_names += $(this).val()+",";
                    });
                }
                rating_names = rating_names.substring(0,rating_names.length - 1);
                question.rating_names = rating_names;
            }
                ...
                $(this).find('.exam-object-tr').each(function(exam_object_index) {
					...
					exam_object.profile = $(this).find('.question-teacher-profile').attr('src');
...
// 설문보관
function save(isSaveAs)
{
    var valid = true;

    if (isSaveAs === undefined) {
        isSaveAs = 0;
    } 
    var survey_id = $('#survey_id').val();
    var is_public_enable = $('#is_public_enable').val();
    var survey_attached = $('#survey_attached').val();
    var attached_file_name = $('#attached_file_name').val();
    var attached_origin_file_name = $('#attached_origin_file_name').val();
    var attached_check = $('#attached_check').val();
    ...
    if($('#newflag').val()==1 || isSaveAs == 0) {
        survey_id = 0;
    }

    if (survey_title == "") {
        swal({title: '', text: '설문제목을 입력해주세요.',
                            confirmButtonText: '확인', allowOutsideClick: false, type: 'warning'},
                        function(isConfirm) {});
        return;
    }
    ...
    $.ajax({
        ...
        url: site_url + 'survey/check_duplicate_name',
        timeout: 5000,
        success: function (result) {
            if (result > 0) {
                swal({
                        title: '', text: '동일한 이름이 있습니다. \n' +
                        '새로운 이름으로 저장하십시요.\n' +
                        '\n' +
                        '그래도 저장하시려면 \n' +
                        '전에 작성한 설문을 삭제하고 저장하십시요.',
                ...
                var show_Type = "0";
                if ($('input:checkbox[name=survey_show_type]').is(':checked'))
                    show_Type = "1";
            
                if (is_public_enable == "0") {
                    isSaveAs = 1;
                }
...
function on_change_group_count(){
    ...
    if (group_count > cur_group_count) {
        ...
        //------------ 그룹번호추가 -------------
            new_group_container.find('.survey-group-number').text('설문그룹' + parseInt(i + 1 + cur_group_count));
                            ...
                            dataType: 'json',
                                ...
                                $.each(data.result.files, function (index, file) {
                                    if(file.error =="File is too big") {
                                        swal({
                                                title: '', text: "이미지파일크기가 2Mb 까지 업로드할수있습니다.",
                                                confirmButtonText: '확인', allowOutsideClick: false, type: 'error'
                                            },
                                            function (isConfirm) {
                                            });
                        ...
                        new_container.find('.exam_object_count').on('change', function() {
                            ...
                            if (exam_object_count > cur_exam_object_count) {
                                for (var i = 0; i < exam_object_count - cur_exam_object_count; i++) {
                                    ...
                                    teacher_tag_id = "education_teacher_" + Number(cur_exam_object_count + i + 1);
                                    teacher_name = $('#' + teacher_tag_id).val();
                                    new_container.find(".exam-object-title").val(teacher_name);

                                    // 프로필 이미지 선택 사건처리
                                    new_container.find('.question-teacher-file').fileupload({
                                        url: site_url + 'survey/upload_img',
                                        survey_container: new_container,
                                        dataType: 'json',
                                        done: function (e, data) {
                                            ...
                                            $.each(data.result.files, function (index, file) {
                                                if(file.error =="File is too big") {
                                                    ...
                                                    var container = data.survey_container;
                                                    add_teacher_img(container, 'survey/thumb_tmp/' + file.name);
                                                }
                                            });
                                        },
                                        progressall: function (e, data) {
                                    ...
                                    // 프로필 이미지 삭제 사건처리
                                    new_container.find('.survey-teacher-img .btn-del-img').on('click', function() {
                                        var container = $(this).parent().parent().parent();
                                        remove_teacher_img(container);
                                    });
                                }
                            }
                        });
...
function change_question_type(container, type) {
    ...
    // 만족도일때
        ...
        //문항이동옵션보여주기
        container.find('.question-type-2 .question-move-index').each(function (index) {
            $(this).show();
        });
        container.find('#reverse_question').show();
    ...
    // 강사만족도일때
        ...
        container.find('#reverse_question').hide();
        //문항이동옵션 숨기기
        container.find('.question-type-2 .question-move-index').each(function (index) {
            $(this).hide();
        });
...
// 사진이미지 추가
function add_teacher_img(container, file_name) {
    if (file_name.substr(0, 1) == "/") {
        file_name = file_name.substr(1);            
    }
    var question_thumb = container.find('.question-teacher-profile');
    question_thumb.attr('src', site_url + file_name);
    question_thumb.removeClass('hidden');
    container.find('.btn-del-img-container').css('display', 'block');
    container.find('.survey-teacher-img .fileinput-button').css('visibility', 'hidden');
    container.find('.survey-teacher-img .fileinput-button').css('height', '0px');
    container.find('.survey-teacher-img .teacher-profile-file-name').val(file_name);
}

// 사진이미지 삭제
function remove_teacher_img(container) {
    var question_thumb = container.find('.question-teacher-profile');
    question_thumb.addClass('hidden');
    question_thumb.attr('src', '');
    container.find('.btn-del-img-container').css('display', 'none');
    container.find('.survey-teacher-img .fileinput-button').css('visibility', 'visible');
    container.find('.survey-teacher-img .fileinput-button').css('height', '100%');
    container.find('.survey-teacher-img .teacher-profile-file-name').val('');
}
...
function on_change_type_grade(container) {
    ...
    if (type_grade_count == 5) {
        ...
        if (type_grade == 0) {
            ...
            container.find('.reverse_question_1').prop('checked',false);
...
function on_change_question_move() {
    ...
    $('.survey-question-container').each(function(question_index) {
        ...
        //만족도(5개짜리)에서 이동옵션변경처리
        $(this).find('.survey-example .type-grade-inputs-5 .question-move').each(function(index) {
            ...
            move_count =  $(this).find('option').length;
            diff = current_value - move_count;
            for ( i=0; i<Math.abs(diff); i++) {
                if (current_value > move_count) {
                    $($.sprintf('<option value="%1$d"> %1$d</option>', move_count + i )).appendTo($(this));

                }
                else if (current_value < move_count) {
                    $(this).find($.sprintf('option:nth-child(%d)', move_count - i)).remove();
                }
            }
            for( i = 0; i< end_check_flag; i++) {
                j = i+1;
                $(this).append("<option value='2"+j+"'>종료 "+j+"</option>");
            }
            $(this).find('option[value=' + selected_value + ']').attr('selected', 'selected');
        });

        //만족도(3개짜리)에서 이동옵션변경처리
        $(this).find('.survey-example .type-grade-inputs-3 .question-move').each(function(index) {
            var selected_value=$(this).val();
...
function on_add_example(example) {
    ...
    var current_value = total_examples;
...
function add_question_move_end(checked,index) {
    ...
    $('.survey-question-container').each(function(question_index) {
        ...
        // 만족도(5개짜리)에서 종료이동 추가
        $(this).find('.survey-example .type-grade-inputs-5 .question-move').each(function(exam_index) {
            move_count =  $(this).find('option').length;
            if(checked) {
                $(this).append("<option value='2"+index+"'>종료 "+index+"</option>");
            } else {
                $(this).find('option:last').remove();
            }
        ...
        // 만족도(3개짜리)에서 종료이동 추가
        $(this).find('.survey-example .type-grade-inputs-3 .question-move').each(function(exam_index) {
            move_count =  $(this).find('option').length;
            if(checked) {
                $(this).append("<option value='2"+index+"'>종료 "+index+"</option>");
            } else {
...
function on_import_question() {
    ...
    if(id !=undefined && id > 0) {
        location.href = site_url + 'survey/view?survey_id=' + id+'&newflag=1&attached='+newflag;
    }
}
...
function preview() {
    ...
    htmldata +='<div class="sub-title"><img src="/images/icon_title.png">설문미리보기</div>';
    ...
    if(preview_attached == 1) {  //문서포함이면
        ...
        htmldata +='<button onclick="invite();" class="btn btn-primary" style=" letter-spacing: 8px;width: 130px;margin-right: 10px;margin-bottom:20px">설문참여</button>';
        htmldata +='</div>';
        htmldata +='<script>';
        htmldata +='$(function () {';
    ...
    $('.survey-group-container').each(function(group_index) {
        ...
        var group_title = $(this).find(".group-title").val();
        if(group_title == ""){
            valid = false;
            swal({
                    title: '', text: $.sprintf('설문그룹%d의 제목을 입력하십시오.', group_index + 1),
                    confirmButtonText: '확인', allowOutsideClick: false, type: 'warning'
                },
                function (isConfirm) {
                });
        ...
        $(this).find('.survey-question-container').each(function () {
            ...
            var allow_random_align = $(this).find('.allow_random_align').is(':checked') ? 1 : 0;
            ...
            var reverse_question = 0;
            if (type == 2) { //만족도식
                reverse_question = $(this).find('.reverse_question_1').is(':checked') ? 1 : 0;
                if (reverse_question == 1) {
                    // condition_text += "(역문항)";
                }
            }
            ...
            if (type == 0) { //객관식
                ...
                if (allow_reply_response == 1) {   //증복응답허용일때
                    ...
                    $(this).find('.survey-example .examples .example').each(function (exam_index) {
                        ...
                        htmldata += '<label>' + (Number(exam_index) + 1) + ') <input type="radio" name="question1_' + question_index + '" role="' + move_value + '" value="' + title + '" isChecked="false" style = "width:17px;height: 17px; margin-top:0">' + title + '</label>';
                ...
                for (i = 0; i < Number(example_count); i++) {
                    example_index = i + 1;
                    htmldata += '<label><input type="text" name="question2_' + i + '" value="" class="example-input-text"></label>';
                }
                ...
                if (type_grade == 0) { //별형
                    htmldata += '<fieldset class="starRating" style = "heigth:30px;width:100%;overflow: hidden">';
                }else if(type_grade == 1) {//원형
                    htmldata += '<fieldset class="circleRating" style = "heigth:30px;width:100%;overflow: hidden">';
                }

                var fav_grades = [];
                if(example_count == 5) {
                    $(this).find('.type-grade-inputs-5 .rating-name').each(function (index) {
                        fav_grades.push($(this).val());
                    });
                }else if(example_count == 3){
                    $(this).find('.type-grade-inputs-3 .rating-name').each(function (index) {
                        fav_grades.push($(this).val());
                    });
                }
                ...
                //문항이동값배열얻기
                var move_value_list = new Array();
                if(example_count == 5) {
                    $(this).find('.question-type-2 .type-grade-inputs-5 .question-move').each(function (index) {
                        move_value_list.push($(this).val());
                    });
                }else if(example_count == 3){
                    $(this).find('.question-type-2 .type-grade-inputs-3 .question-move').each(function (index) {
                        move_value_list.push($(this).val());
                    });
                }
                ...
                if (type_grade == 0) { //별형
                    ...
                    var checked_prop = "checked";
                    for (i = Number(example_count); i > 0; i--) {
                        star_header += '<td class="matrix-col-label" style="width:' + width_rate + '%">' + fav_grades[j - i] + '</td>';

                        star_section += '<input class="star-input" id="' + question_index + '_rating' + i + '" type="radio" name="rating"'+ '" role="' + move_value_list[i - 1] + '" value="' + fav_grades[i - 1] + '" ' + checked_prop + '>';
                        star_section += '<label class="star-label" for="' + question_index + '_rating' + i + '" style = "width:' + itemWidth + '%"></label>';
                    ...
                    for (i = Number(example_count); i > 0; i--) {
                        ...
                        star_section += '<input class="circle-label" id="' + question_index + '_rating' + i + '" type="radio" name="rating"' + '" role="' + move_value_list[i - 1] + '" value="' + fav_grades[i - 1] + '" ' + checked_prop + '>';
                        star_section += '<label class="circle-label" for="' + question_index + '_rating' + i + '" style = "width:' + itemWidth + '%"></label>';
                ...
                htmldata += ' </fieldset>';
                // htmldata += '<div style = "font-size: 14px; margin-left: 25px;">';
                // htmldata += '1,2번으로 답하실 경우 불만족한 이유가 무엇입니까?';
                // htmldata += '<textarea class="teacher_satisfy_reason form-control"  style="resize: none"></textarea>';                
                // htmldata += '</div></form>';
                htmldata += '</form>';
            }
                ...
                //평가대상별 만족도구현
                $(this).find(".exam-object-tr").each(function(exam_object_index) {
                    var profile_src = $(this).find('.question-teacher-profile').attr('src');
                    imgtag = "";    
                    if (profile_src != "") {
                        imgtag = "<img src=" + profile_src + " style='width: 50px;'>";    
                    }                    
                    htmldata += "<div style='font-size: 20px;;margin: 10px 0;padding: 7px;background: #eaeaea;'>" + imgtag + "< "
                    ...
                    $(this).closest('.survey-question-container').find('.exam-kind-tr').each(function(exam_kind_index) {
                        ...
                        if (type_grade == 0) { //별형
                            ...
                            for (i = Number(example_count); i > 0; i--) {
                                ...
                                star_section += '<input class="star-input" id="' + question_index +"_" + exam_object_index + "_" + exam_kind_index + '_rating' + i + '" type="radio" name="rating_"' + exam_object_index + "_" + exam_kind_index + ' value="' + fav_grades[i-1] + '" ' + checked_prop + '>';
                                star_section += '<label class="star-label" for="' + question_index +"_" + exam_object_index + "_" + exam_kind_index + '_rating' + i + '" style = "width:' + itemWidth + '%"></label>';
                            ...
                            for (i = Number(example_count); i > 0; i--) {
                                ...
                                star_section += '<input class="circle-input" id="' + question_index +"_" + exam_object_index + "_" + exam_kind_index + '_rating' + i + '" type="radio" name="rating_"' + exam_object_index + "_" + exam_kind_index + 'value="' + fav_grades[i-1] + '" ' + checked_prop + '>';
                                star_section += '<label class="circle-label" for="' + question_index +"_" + exam_object_index + "_" + exam_kind_index + '_rating' + i + '" style = "width:' + itemWidth + '%"></label>';
                    ...
                    htmldata += '<label>&nbsp;&nbsp;강사님에 대한 의견을 남겨주세요.<br>';
                    htmldata += '<input type="text" name="teacher_desc" value="" class="example-input-text"></label>';
                });
            }
    ...
    htmldata +='<div style="text-align: center; margin-top: 19px;">';
    htmldata +='<button onclick="before();" class="btn_auth_ok btn " style=" letter-spacing: 8px;margin-right: 10px;margin-bottom:20px;display: none" id="button_before">이전</button>';
    htmldata +='<button onclick="next();" class="btn_auth_ok btn " style=" letter-spacing: 8px;margin-right: 10px;margin-bottom:20px" id="button_next">다음</button>';
    htmldata +='<button onclick="previewClose_check();" class="btn_auth_ok btn " style="letter-spacing: 8px;margin-bottom:20px" id ="button_close">돌아가기</button>';
    htmldata +='</div>';
            ...
            if ($(this).find('#allow_unselect').val() == "0") {
                ...
                if ($(this).find('#type').val() == "0") {
                    ...
                    if ($(this).find('#type_grade').val() == "2") {
                        ...
                        var checkedRadio = $(this).find('input[name="rating"]:checked');
...
function onImportClick() {
    ...
    if($('#attached_check').val() == 1)
    {
        ...
        $.ajax({
            url: site_url + 'survey/convertToHTML',
            type: 'POST',
            data: {
...
function setData(loaded_page) {
    var surveyData= [];
    var questionData = [];
    var end_comments =[];

    $.post(site_url + 'survey/get_surveyById',
    {
        survey_id: survey_id
    },
        function(data, status){
            ...
            if (obj.status == 'OK') {
                ...
                if(surveyData.length>0) {
                    ...
                    if (loaded_page == 0) {
                        //설문제목설정
                        $('#education_title').val(surveyData[0]['title']);
                        //설문기간설정
                    ...
                    total_examples = 0;
                    $('.survey-group-container').each(function (group_index) {
                        total_examples += parseInt(g_question_groups[group_index]['question_count']);
                    });
                    ...
                    $('.survey-group-container').each(function (group_index) {
                        ...
                        $(this).find('.survey-question-container').each(function (question_index) {
                            ...
                            if (g_questions[question_index]['question_img_url'] != '') {
                                add_question_img(g_question_container, 'survey/thumb/' + g_questions[question_index]['question_img_url']);
                            }
                            else {
                            ...
                            // 문항형태가 만족도일때
                                ...
                                var rating_move_value = g_questions[question_index]['rating_move_value'];

                                if(rating_move_value != null){
                                    var lst_move_value = rating_move_value.split(',');

                                    if(g_questions[question_index]['example_count'] == 5) {
                                        $(this).find('.question-type-2 .type-grade-inputs-5 .question-move').each(function (index) {
                                            $(this).find('option[value=' + lst_move_value[index] + ']').attr('selected', 'selected');
                                        });
                                    }else if(g_questions[question_index]['example_count'] == 3){
                                        $(this).find('.question-type-2 .type-grade-inputs-3 .question-move').each(function (index) {
                                            $(this).find('option[value=' + lst_move_value[index] + ']').attr('selected', 'selected');
                                        });
                                    }
                                ...
                                //만족도항목값들 설정
                                var rating_names = g_questions[question_index]['rating_names'];
                                if(rating_names != null){
                                    var lst_rating_names = rating_names.split(',');

                                    if(g_questions[question_index]['example_count'] == 5) {
                                        $(this).find('.question-type-2 .type-grade-inputs-5 .rating-name').each(function (index) {
                                            $(this).val(lst_rating_names[index]);
                                        });
                                    }else if(g_questions[question_index]['example_count'] == 3){
                                        $(this).find('.question-type-2 .type-grade-inputs-3 .rating-name').each(function (index) {
                                            $(this).val(lst_rating_names[index]);
                                        });
                                    }
                                }
                                ...
                                if (g_questions[question_index]['reverse_question'] == 1) {
                                    g_question_container.find('input:checkbox[name="reverse_question_1"]').prop("checked", true);
                                } else {
                                    g_question_container.find('input:checkbox[name="reverse_question_1"]').prop("checked", false);
                                }
                            ...
                            // 문항형태가 강사만족도일때
                                ...
                                //      profile_img = exam_objects[object_index]['profile'];
                                //      if (profile_img != "") {
                                //         var img_container = $(this).find('.question-teacher-profile').parent().parent();    
                                //         add_teacher_img(img_container, profile_img);
                                //      }                                     
                                // });
                            }
                        });
...
function edit_education() {
    ...
    var attached = $("#survey_attached").val();
    location.href = site_url + 'survey/survey_list/public?survey_id='+surveyid+'&newflag='+newflag+'&attached='+attached;
}
...
function browse_survey() {    
    ...
    if($("#education_customer").css("visibility") != "hidden") {
        var education_course = $("#education_caption").val();
        var education_customer = $("#education_customer").val();
        var education_teacher = $("#education_teacher_1").val() + ',' +
                                $("#education_teacher_2").val() + ',' +
    ...
    parameter = "";
    parameter += "prev_education_id=" + education_id;
    parameter += "&education_title=" + education_title;
    parameter += "&survey_start_date=" + survey_start_date;
    parameter += "&survey_end_date=" + survey_end_date;
    if($("#education_customer").css("visibility") != "hidden") {
        parameter += "&sms_available=1";
        parameter += "&education_course=" + education_course;
        parameter += "&education_customer=" + education_customer;
        parameter += "&education_teacher=" + education_teacher;
    }else
        parameter += "&sms_available=0";

    //공개이면
    if(survey_flag == 1)
        location.href = site_url + 'survey/load_list/public?' + parameter;
    else
        location.href = site_url + 'survey/load_list/advanced?' + parameter;
}
