...
<form name="emo_frm">
    <label for="emo_sel" class="sound_only">내문자 그룹</label>
...
<select name="st" id="st">
    <option value="all"<?php echo get_selected('all', $st); ?>>제목 + 내 메세지</option>
    <option value="name"<?php echo get_selected('name', $st); ?>>제목</option>
    <option value="content"<?php echo get_selected('content', $st); ?>>내 메세지</option>
</select>
...
(function($){
    ...
    emoticon_list.select_page = function( params, type ){
        ...
        emoticon_list.loading(".emo_list", "/service/images/ajax-loader.gif" ); //로딩 이미지 보여줌
        $.ajax({
            url: "/service/ajax.sms_write_form.php",
            cache:false,
            timeout : 30000,
            dataType:type,
            data:params,
