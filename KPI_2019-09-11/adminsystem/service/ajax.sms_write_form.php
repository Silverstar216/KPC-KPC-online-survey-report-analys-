...
$total_res = sql_fetch("select count(*) as cnt from {$g5['sms5_form_table']} where fg_member = '{$member['mb_no']}' $sql_group $sql_search");
...
$group = array();
$qry = sql_query("select * from {$g5['sms5_form_group_table']} where fg_member in ('','{$member['mb_no']}') order by fg_name");
while ($res = sql_fetch_array($qry)) array_push($group, $res);

$res = sql_fetch("select count(*) as cnt from {$g5['sms5_form_table']} where fg_member = '{$member['mb_no']}' and fg_no=0");
$no_count = $res['cnt'];

$count = 1;
$qry = sql_query("select * from {$g5['sms5_form_table']} where fg_member = '{$member['mb_no']}' $sql_group $sql_search order by fo_no desc limit $page_start, $page_size");
$list_text = "";
