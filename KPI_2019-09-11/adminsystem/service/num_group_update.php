...
if ($w == 'u') // 업데이트
{
    for ($i=0; $i<count($_POST['chk']); $i++)
    {
        ...
        $res = sql_fetch("select bg_name from {$g5['sms5_book_group_table']} where bg_no<>'$bg_no' and bg_member = '{$member['mb_no']}' and bg_name='$bg_name'");
        if ($res)
            alert('같은 그룹명이 존재합니다.');
...
else if ($w == 'em') // 비우기
{
    for ($i=0; $i<count($_POST['chk']); $i++)
    {
        ...
        sql_query("update {$g5['sms5_book_group_table']} set bg_count = 0,bg_nomember = 0, bg_reject = 0 where bg_no='$bg_no'");
        sql_query("delete from {$g5['sms5_book_table']} where bg_no='$bg_no'");
    }
}
else // 등록
{
    ...
    $res = sql_fetch("select bg_name from {$g5['sms5_book_group_table']} where bg_name='$bg_name' and bg_member = '{$member['mb_no']}'  ");
    if ($res)
        alert('같은 그룹명이 존재합니다.');

    sql_query("insert into {$g5['sms5_book_group_table']} set bg_name='".addslashes($bg_name)."', bg_member = '{$member['mb_no']}' ");
}
