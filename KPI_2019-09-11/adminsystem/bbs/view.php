...
//    $list_href = './board.php?bo_table='.$bo_table;
    $search_href = './board?bo_table='.$bo_table.'&amp;page='.$page.$qstr;
    $list_href = './board?bo_table='.$bo_table;
} else {
    $search_href = '';
//    $list_href = './board.php?bo_table='.$bo_table.'&amp;page='.$page;
    $list_href = './board?bo_table='.$bo_table.'&amp;page='.$page;
}
...
//    $prev_href = './board.php?bo_table='.$bo_table.'&amp;wr_id='.$prev['wr_id'].$qstr;
    $prev_href = './board?bo_table='.$bo_table.'&amp;wr_id='.$prev['wr_id'].$qstr;
}
...
//    $next_href = './board.php?bo_table='.$bo_table.'&amp;wr_id='.$next['wr_id'].$qstr;
    $next_href = './board?bo_table='.$bo_table.'&amp;wr_id='.$next['wr_id'].$qstr;
}
...
//    $write_href = './write.php?bo_table='.$bo_table;
    $write_href = './board_write?bo_table='.$bo_table;
// 답변 링크
$reply_href = '';
if ($user_level >= $board['bo_reply_level'])
//    $reply_href = './write.php?w=r&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr;
    $reply_href = './board_write?w=r&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr;

// 수정, 삭제 링크
$update_href = $delete_href = '';
// 로그인중이고 자신의 글이라면 또는 관리자라면 비밀번호를 묻지 않고 바로 수정, 삭제 가능
//=============SGS=============
if (($user_uid && ($user_uid == $write['mb_id'])) || $is_admin) {
    $update_href = './board_write?w=u&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    $delete_href = G5_DOMAIN.'bbs/delete.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.urldecode($qstr);
    if ($is_admin)
    {
        set_session("ss_delete_token", $token = uniqid(time()));
        $delete_href =G5_DOMAIN.'bbs/delete.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;token='.$token.'&amp;page='.$page.urldecode($qstr);
    }
}
else if (!$write['mb_id']) { // 회원이 쓴 글이 아니라면
    $update_href = G5_DOMAIN.'/bbs/password.php?w=u&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    $delete_href = G5_DOMAIN.'/bbs/password.php?w=d&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
}
