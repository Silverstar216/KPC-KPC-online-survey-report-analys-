<div class="m7con" style = "text-align: center;">
<?php
...
if ($w == '') {
    ...
    if ($user_level < $board['bo_write_level']) {
        if ($user_id) {
            alert('글을 쓸 권한이 없습니다.');
        } else {
            alert("글을 쓸 권한이 없습니다.\\n회원이시라면 로그인 후 이용해 보십시오.", Main_DOMAIN.'join/login_view');
            return;
//            alert("글을 쓸 권한이 없습니다.\\n회원이시라면 로그인 후 이용해 보십시오.", 'broad?'.$qstr.'&amp;url='.urlencode($_SERVER['PHP_SELF'].'?bo_table='.$bo_table));
        }
    }
    ...
    if($user_id && $write['mb_id'] == $user_id) {
        ;
    } else if ($user_level < $board['bo_write_level']) {
        if ($user_id) {
            alert('글을 수정할 권한이 없습니다.');return;
        } else {
...
//            alert('글을 수정할 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', './login.php?'.$qstr.'&amp;url='.urlencode($_SERVER['PHP_SELF'].'?bo_table='.$bo_table));
    ...
    if ($user_level < $board['bo_reply_level']) {
        if ($user_id)
    ...
    if (strstr($write['wr_option'], 'secret')) {
        if ($write['mb_id']) {
            ...
            if (!($write['mb_id'] == $user_id || $is_admin))
...
if (!empty($group['gr_use_access'])) {
    ...
    if ($is_admin == 'super' || $group['gr_admin'] == $user_id || $board['bo_admin'] == $user_id) {
        ; // 통과
    } else {
        // 그룹접근
        $sql = " select gr_id from {$g5['group_member_table']} where gr_id = '{$board['gr_id']}' and mb_id = '{$user_id}' ";
        $row = sql_fetch($sql);
        if (!$row['gr_id'])
...
$is_html = false;
if ($user_level >= $board['bo_html_level'])
    $is_html = true;
...
$is_email    = false;
$is_homepage = false;
if ($is_guest || ($is_admin && $w == 'u' && $user_id != $write['mb_id'])) {
    $is_name = true;
    $is_password = true;
    $is_email = true;
    $is_homepage = true;
...
$is_link = false;
if ($user_level >= $board['bo_link_level']) {
    $is_link = true;
}

$is_file = false;
if ($user_level >= $board['bo_upload_level']) {
    $is_file = true;
}
...
if ($w == '') {
    ...
    if (!$is_admin) {
        if (!($is_member && $user_id == $write['mb_id'])) {
...
if ($config['cf_editor'] && !G5_IS_MOBILE && $board['bo_use_dhtml_editor'] && $user_level >= $board['bo_html_level']) {
    $is_dhtml_editor = true;
}
$editor_html = editor_html('wr_content', $content, $is_dhtml_editor,$config['cf_editor']);
$editor_js = '';
$editor_js .= get_editor_js('wr_content', $is_dhtml_editor);
$editor_js .= chk_editor_js('wr_content', $is_dhtml_editor);

// 임시 저장된 글 수
$autosave_count = autosave_count($user_id);

include_once(G5_PATH.'/head.sub.php');
//@include_once ($board_skin_path.'/write.head.skin.php');
//include_once('./board_head.php');

$action_url = https_url(G5_BBS_DIR)."/write_update.php?ss_bo_table=".$_REQUEST['bo_table']."&ss_wr_id=".$_REQUEST['wr_id'];
