...
$root = $_SERVER['SERVER_NAME'].'/adminsystem';
...
if ($user_uid) {
    $is_member = true;
    if ($config['cf_admin'] == $user_uid) $is_admin= 'super';
    if (isset($group['gr_admin']) && ($group['gr_admin'] == $user_uid)) $is_admin= 'group';
    if (isset($board['bo_admin']) && ($board['bo_admin'] == $user_uid)) $is_admin= 'board';
    $member['mb_dir'] = substr($user_uid,0,2);
} else {
    $is_guest = true;
    $member['mb_id'] = '';
}
@session_start();
$aaa = $_SESSION['ss_mb_id'];
if (!$board['bo_table']) {
...
if (isset($wr_id) && $wr_id) {
    ...
    if (isset($group['gr_use_access']) && $group['gr_use_access']) {
        ...
        if ($is_admin == "super" || $is_admin == "group") {
            ...
            // 그룹접근
            $sql = " select count(*) as cnt from {$g5['group_member_table']} where gr_id = '{$board['gr_id']}' and mb_id = '{$user_id}' ";
            $row = sql_fetch($sql);
            if (!$row['cnt']) {
...
//    if ($user_level < $board['bo_read_level']) {
//        if ($is_member)
...
//    }
    ...
    if (!get_session($ss_name))
    {
        ...
        // 자신의 글이면 통과
        if ($write['mb_id'] && $write['mb_id'] == $user_id) {
            ;
            ...
            insert_point($user_id, $board['bo_read_point'], "{$board['bo_subject']} {$wr_id} 글읽기", $bo_table, $wr_id, '읽기');
        }
...
//    if ($user_level < $board['bo_list_level']) {
//        if ($user_id)
...
// 최고관리자 또는 그룹관리자라면
if ($user_id && ($is_admin == 'super' || $group['gr_admin'] == $user_id))
...
//    include_once('./view.php');
    include_once (FCPATH.'adminsystem/bbs/view.php');
}

// 전체목록보이기 사용이 "예" 또는 wr_id 값이 없다면 목록을 보임
if ($board['bo_use_list_view'] || empty($wr_id))
if ($user_level >= $board['bo_list_level'] && $board['bo_use_list_view'] || empty($wr_id))
    include_once (FCPATH.'adminsystem/bbs/list.php');
//include_once (FCPATH.'adminsystem/bbs/board_tail.php');
