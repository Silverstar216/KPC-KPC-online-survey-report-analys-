...
$mb_old_tel     = isset($_POST['mb_old_tel'])            ? trim($_POST['mb_old_tel'])          : "";
if ($w == '' || $w == 'u') {
...
if ($w == '') {
    ...
    $sql = "select * from ele_price where elpe_ukey = '1'";
    $eleprice_sample = sql_fetch($sql);
	
	$sql = "select mb_no from {$g5['member_table']} where mb_id = '{$mb_id}'";
    $result = sql_fetch($sql);
	
    if ($result) {        
			$elem_sms_user_price = $eleprice_sample['elpe_sms_money'];
			$elem_lms_user_price = $eleprice_sample['elpe_lms_money'];
			$elem_smsdoc_user_price = $eleprice_sample['elpe_mms_money'];
			$elem_lmsdoc_user_price = $eleprice_sample['elpe_cv_money'];
			
            $mb_first_no = $result['mb_no'];   
            $sql = "insert into ele_money_mst ";    
            $sql .= "(elem_id,elem_stat,elem_type,elem_crnt_money,elem_money,elem_crnt_cv_bonus_cnt,elem_sms_user_price,elem_lms_user_price,elem_smsdoc_user_price,elem_lmsdoc_user_price,elem_proc_time) ";
            $sql .= " values ('{$mb_first_no}','Y','001',".
                       "ifnull((select min(elpu_money) from ele_price_user where elpu_type = '001' and elpu_stat = 'm'),0),".
					   "ifnull((select min(elpu_money) from ele_price_user where elpu_type = '001' and elpu_stat = 'm'),0),".
                       "ifnull((select min(elpu_cv_limit_count) from ele_price_user where elpu_type = '001' and elpu_stat = 'm'),0),".
					   "{$elem_sms_user_price}, {$elem_lms_user_price}, {$elem_smsdoc_user_price}, {$elem_lmsdoc_user_price},".
                       "sysdate())";        
            sql_query($sql);

            $sql = "update {$g5['member_table']} set mb_owner = '{$mb_first_no}' where mb_id = '{$mb_id}'";
            sql_query($sql);            

            if ($mb_tel == ''){

            } else {
                    $sql = "insert into sms5_phone_identity (ph_mbno,ph_phone,ph_identity,ph_idate,ph_gubn,ph_cdate,ph_bigo) values ".
                                    "('{$mb_first_no}','{$mb_tel}',0,sysdate(),1,sysdate(),'{$mb_nick}')";     
                    sql_query($sql);                       
            }  
    }      
    // 회원가입 포인트 부여
    ...
    if ($mb_old_tel == $mb_tel) {

    } else {
        if ($mb_tel == ''){

        } else {
            $res = sql_fetch("select * from sms5_phone_identity where ph_mbno = '{$member['mb_no']}' and ph_phone = '{$mb_tel}' ");
            if ($res)   {
                if ($res['ph_gubn'] == '5'){
                ...
                $SSql = "insert into sms5_phone_identity (ph_mbno,ph_phone,ph_identity,ph_idate,ph_gubn,ph_cdate,ph_bigo) values ".
                        "('{$member['mb_no']}','{$mb_tel}',0,sysdate(),1,sysdate(),'{$mb_nick}')";
                sql_query($SSql);
            } 
        }
    }
