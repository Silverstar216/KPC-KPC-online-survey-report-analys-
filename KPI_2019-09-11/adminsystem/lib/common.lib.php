...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $str = '';
    if ($cur_page > 1) {
        $str .= '<a href="'.$url.'1'.$add.'" class="pg_page pg_start"><img src="'.G5_IMG_URL.'/btn_board_pprev.png" border="0" align=absmiddle title="처음"></a>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<a href="'.$url.($start_page-1).$add.'" class="pg_page pg_prev"><img src="'.G5_IMG_URL.'/btn_board_prev.png" border="0" align=absmiddle title="이전"></a>'.PHP_EOL;
    ...
    if ($total_page > $end_page) $str .= '<a href="'.$url.($end_page+1).$add.'" class="pg_page pg_next"><img src="'.G5_IMG_URL.'/btn_board_next.png" border="0" align=absmiddle title="다음"></a>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<a href="'.$url.$total_page.$add.'" class="pg_page pg_end"><img src="'.G5_IMG_URL.'/btn_board_nnext.png" border="0" align=absmiddle title="맨끝"></a>'.PHP_EOL;
    }

    if ($str)
        return "<nav class=\"pg_wrap\" style = 'padding:0 0'><span class=\"pg\">{$str}</span></nav>";
    else
        return "";
}
...
//    $tmp_name = get_text(cut_str($list['wr_name'], $config['cf_cut_name'])); // 설정된 자리수 만큼만 이름 출력
    ...
    // 분류명 링크
    $list['ca_name_href'] = Main_DOMAIN.'index/board?bo_table='.$board['bo_table'].'&amp;sca='.urlencode($list['ca_name']);

    $list['href'] = Main_DOMAIN.'index/board?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].$qstr;
...
function get_member_link($mb_id, $fields='*')
{
    global $g5;

    return sql_query(" select $fields from g5_member_link where mb_id = TRIM('$mb_id') ");
}

// 입금 정보를 얻는다.
function get_member_money($mb_id, $fields='*')
{
    global $g5;

    return sql_fetch("select $fields from user_money, {$g5['member_table']} where mb_id = TRIM('$mb_id') and mb_no = user_id");
}
//입금이력정보를 얻는다
function get_member_money_history($mb_id, $fields='*')
{
    global $g5;
    return sql_query("select $fields from user_money_history, {$g5['member_table']} where mb_id = TRIM('$mb_id') and mb_no = user_id");
}
...
function get_sideview($mb_id, $name='', $email='', $homepage='')
{
   ...
   /* if($homepage)
    ...
    $mb = get_member($mb_id, " mb_hp ,mb_name");
    $str2 .= "<a href=\"/notice?m=".$mb['mb_hp']."&n=".$mb['mb_name']."/\" >문자보내기</a>\n";
...
function sql_query($sql, $error=G5_DISPLAY_SQL_ERROR,$kind = 0,$g5_param = [])
{
    global $g5;
    if($kind == 1)
        $g5 = $g5_param;
    ...
    if ($error)
        $result = @mysqli_query($g5['connect_db'], $sql) or die("<p>$sql<p>" . mysqli_errno() . " : " .  mysqli_error() . "<p>error file : {$_SERVER['PHP_SELF']}");
    else
...
function sql_query_emma($db, $sql, $error=G5_DISPLAY_SQL_ERROR)
{
    global $g5;
...
function sql_fetch($sql, $error=G5_DISPLAY_SQL_ERROR,$kind = 0,$g5_param = [])
{
    if($kind == 1)
        $result = sql_query($sql, $error, $kind, $g5_param);
    else
...
// 쿼리를 실행한 후 결과값에서 한행을 얻는다.
function sql_fetch_emma($db, $sql, $error=G5_DISPLAY_SQL_ERROR)
{
    $result = sql_query_emma($db, $sql, $error);
    $row = sql_fetch_array($result);
    return $row;
}
...
function add_stylesheet($stylesheet, $order=0,$kind = 0,$htmlInstance = null)
{
    global $html_process;
    if($kind == 1)
        $html_process = $htmlInstance;
    if(trim($stylesheet))
...
function survey_delete($survey_id)
{
    ...
    sql_query($sql);
    // $sql = "delete from survey_end_template where survey_id= '".$survey_id."' ";
    // sql_query($sql);
    $sql = "select * id from questions where where survey_id= '".$survey_id."' ";
    $result = sql_query($sql);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        $sql_1 = "delete from examples where question_id= '".$row['id']."' ";
        sql_query($sql_1);
    }
    $sql = "delete from questions where survey_id= '".$survey_id."' ";
    sql_query($sql);
    /* $sql = " select mb_name, mb_nick, mb_ip, mb_recommend, mb_memo, mb_level from {$g5['member_table']} where mb_id= '".$mb_id."' ";
     $mb = sql_fetch($sql);
     if ($mb['mb_recommend']) {
...
function get_icode_userinfo($id, $pass)
{
    ...
    */
    $userinfo = array(
        'code'      => '', // 결과코드
        'coin'      => '', // 고객 잔액 (충전제만 해당)
        'gpay'      => '', // 고객의 건수 별 차감액 표시 (충전제만 해당)
        'payment'   => 'C'  // 요금제 표시, A:충전제, C:정액제
    );
    return $userinfo;
}

// 충전모드 텍스트
function get_chargetype_text($type)
{
    $type_text = "미설정";
    if ($type == 1)
        $type_text = "선불";
