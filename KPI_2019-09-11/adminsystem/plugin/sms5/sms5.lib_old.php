		...
		while ($hisrow = sql_fetch_array($qry))
		{
			...
			if ($lmsvalue == 'LMS') {				
				if ($isbundle == true) {
					...
					// 개별전송
					if ($dateclientreq == "")
						$sql = "insert into em_mmt_tran (date_client_req, subject, content, attach_file_group_key, callback, service_type, broadcast_yn, msg_status, recipient_num) values"."(sysdate(), '{$subject}', '{$content}', '0', '{$callback}', '3', 'N', '1', '{$phone}');";
					else
						$sql = "insert into em_mmt_tran (date_client_req, subject, content, attach_file_group_key, callback, service_type, broadcast_yn, msg_status, recipient_num) values"."('{$dateclientreq}', '{$subject}', '{$content}', '0', '{$callback}', '3', 'N', '1', '{$phone}');";					
					sql_query($sql);
				...
				if ($isbundle == true) {
					...
					if (($wr_type == 0)||($wr_type == 1)||($wr_type == 2)) {                                // short_url 생성                         
						sql_query("update sms5_history set hs_message ='{$content}',hs_lurl = '{$mmlong_url}',hs_surl='{$S_url_nm}', hs_mt_pr = '{$max_smt}' where hs_no='{$hs_no}' ");                       
					} else {
						sql_query("update sms5_history set hs_message ='{$content}', hs_mt_pr = '{$max_smt}' where hs_no='{$hs_no}' ");                                               
					}   							
