...
else if ($w == 'u')
{
	...
	$mb['mb_chargetype'] = get_text($mb['mb_chargetype']);  
}
else
...
add_javascript(G5_POSTCODE_JS, 0);    //다음 주소 js
  ...
  $Mb_7 = json_decode($mb['mb_7']);
  if ($Mb_7->{'LMS'} == '1'){
        $mb_lms_yes         =  'checked="checked"';
        $mb_lms_no          = '';
  } else {
        $mb_lms_yes         =  '';
        $mb_lms_no          = 'checked="checked"';
  }
  $Mb_8 = json_decode($mb['mb_8']);
  if ($Mb_8->{'GOJI'} == '1'){
        $mb_goji_yes         =  '';
        $mb_goji_no          = 'checked="checked"';    
  } else {
        $mb_goji_yes         =  'checked="checked"';
        $mb_goji_no          = '';    
  }  
?>
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="row"><label for="mb_nick">기관명<strong class="sound_only">필수</strong></label></th>
...
this.form.mb_intercept_date.value=this.form.mb_intercept_date.defaultValue; }">
    ...
    <tr>
        <th scope="row"><label for="mb_lms_yes">LMS사용 여부</label></th>
        <td>
            <input type="radio" name="mb_lms" value="1" id="mb_lms_yes" <?php echo $mb_lms_yes; ?>>
            <label for="mb_lms_yes">예</label>
            <input type="radio" name="mb_lms" value="0" id="mb_lms_yes" <?php echo $mb_lms_no; ?>>
            <label for="mb_lms_no">아니오</label>
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="mb_goji_yes">개별 고지 기본값</label></th>
        <td>
            <input type="radio" name="mb_goji" value="7" id="mb_goji_yes" <?php echo $mb_goji_yes; ?>>
            <label for="mb_goji_yes">에듀파인</label>
            <input type="radio" name="mb_goji" value="1" id="mb_goji_yes" <?php echo $mb_goji_no; ?>>
            <label for="mb_goji_no">개별양식</label>
        </td>
