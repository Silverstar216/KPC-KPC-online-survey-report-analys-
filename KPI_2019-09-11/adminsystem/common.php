...
if (file_exists($dbconfig_file)) {
    ...
    $connect_db = sql_connect(G5_MYSQL_HOST, G5_MYSQL_USER, G5_MYSQL_PASSWORD, G5_MYSQL_DB) or die('MySQL Connect Error!!!');
    $select_db  = sql_select_db(G5_MYSQL_DB, $connect_db) or die('MySQL DB Error!!!');

    // mysql connect resource $g5 배열에 저장 - 명랑폐인님 제안
    $g5['connect_db'] = $connect_db;

    sql_query(" set names utf8 ",'',1,$g5);
    if (defined('G5_MYSQL_SET_MODE') && G5_MYSQL_SET_MODE) sql_query("SET SESSION sql_mode = ''",'',1,$g5);
    if (defined('G5_TIMEZONE')) sql_query(" set time_zone = '".G5_TIMEZONE."'",'',1,$g5);
} else {
    ?>
...
$config = sql_fetch(" select * from {$g5['config_table']} ",'',1,$g5);
...
if (is_mobile()) {
    define('G5_IS_REAL_MOBILE', true);
} else {
    define('G5_IS_REAL_MOBILE', false);
}
...
if ($_SESSION['ss_mb_id']) { // 로그인중이라면
    ...
    if ($tmp_mb_id = get_cookie('ck_mb_id')) {
        ...
        if (strtolower($tmp_mb_id) != strtolower($config['cf_admin'])) {
            $sql = " select mb_password, mb_intercept_date, mb_leave_date, mb_email_certify,mb_no from {$g5['member_table']} where mb_id = '{$tmp_mb_id}' ";
            $row = sql_fetch($sql,'',1,$g5);
...
//
//$command = 'start /B php C:/inetpub/wwwroot/auto/auto_conv_for_school.php '.$row['mb_no'].' eiajldzxnsdasd12131caqw > NUL';
...
$write_table = "";
if ($bo_table) {
    $board = sql_fetch(" select * from {$g5['board_table']} where bo_table = '$bo_table' ",'',1,$g5);
    if ($board['bo_table']) {
        ...
        if (isset($wr_id) && $wr_id)
            $write = sql_fetch(" select * from $write_table where wr_id = '$wr_id' ",'',1,$g5);
    }
}

if ($gr_id) {
    $group = sql_fetch(" select * from {$g5['group_table']} where gr_id = '$gr_id' ",'',1,$g5);
}
