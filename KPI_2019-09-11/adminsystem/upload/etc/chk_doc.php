<?php
	...
	$nextUrl = G5_URL.'/upload/etc/eletter.php?ep='.$ep.'&sk='.$sk;	
	if ($ep =='') {
		$nextUrl = G5_URL;
	} else if ($sk == '') {
		...
		if ($hs['hs_hp']) {
			...
			if ($end4n == $mnumber){
				...
				$nextUrl = G5_URL.'/upload/etc/eletter.php?ep='.$ep.'&emsg='.urlencode('수신 전화번호 끝자리 4자리(미등록시 0000)를 입력하세요!');				
			}			
		} else {
			$nextUrl = G5_URL.'/upload/etc/eletter.php?ep='.$ep.'&emsg='.urlencode('수신 전화번호 끝자리 4자리(미등록시 0000)를 입력하세요!');							
		}
		...
		if ($hs['hs_hp']) {
			...
			if ($end4n == $mnumber){
				...
				$nextUrl = G5_URL.'/upload/etc/eletter.php?ep='.$ep.'&sk='.$sk.'&emsg='.urlencode('끝자리를 다시 확인하세요.');				
			}			
		} else {
			$nextUrl = G5_URL.'/upload/etc/eletter.php?ep='.$ep.'&sk='.$sk.'&emsg='.urlencode('끝자리를 다시 확인하세요.');							
		}
