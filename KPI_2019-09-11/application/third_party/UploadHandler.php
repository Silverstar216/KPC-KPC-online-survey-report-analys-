...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/uploads/tmp/',
            'upload_url' => $this->get_full_url().'/uploads/tmp/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    function get_config_bytes($val) {
        $valNumber = (int) substr(trim($val), 0, -1);
        $last = strtolower($val[strlen($val)-1]);

        if(is_numeric($last)) {
            $valNumber = (int) $valNumber;
        } else {
            $valNumber = (int) substr(trim($val), 0, -1);
        }
        ...
        return $this->fix_integer_overflow($valNumber);
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if(stripos($name, '.') === false) {
            ...
            $name = time() . substr($name, stripos($name, '.'));
        }
