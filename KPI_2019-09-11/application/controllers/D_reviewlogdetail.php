...
class D_reviewlogdetail extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->data['scripts'] = array(
            ...
            'include/js/index.js',
            'include/js/log/d_reviewlogdetail.js',
        );
...
//        $user_id = 1;
     ...
     /*   if (empty($start_date))
        ...
        //한개 survey를 꺼내기
        $survey = $this->diagnosis_model->get_data_by_noticeId($notice_id);
        $this->data['survey'] = $survey[0];

        //미응답자들을 얻기
        $noResponseMobiles = $this->D_reviews_model->get_noResponseMobile($notice_id,$user_id);
        $this->data['noResponseMobiles'] = $noResponseMobiles;

        //설문안의 질문들과 모든 문항들을 얻기
        $questions = $this->D_questions_model->getExampleData($survey[0]['id']);
        $this->data['questions'] = $questions;

        //설문에 대한 응답통계자료들을 얻기
        $allReview = $this->D_reviews_model->get_allReview($notice_id,$questions,$user_id);
        $this->data['allReview'] = $allReview;

        $response_count = $this->D_reviews_model->get_response_count($notice_id);
        ...
        $this->load->view('log/d_reviewlogdetail', $this->data);
        $this->load->view('log/d_reviewdetail_modal', $this->data);
...
//        $this->excel->getActiveSheet()->getStyle('B')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            ...
            $this->excel->getActiveSheet()->getStyle('A1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB("debf87");
            $this->excel->getActiveSheet()->getStyle('A1')->getFont()->setBold(true);
            ...
            $content = ['설문일자',$title['date'],'발송수',$title['mobile_count'],'응답수',$title['response_count'],'담당자자',$title['man']];
            for($i = 0;$i < 8; $i ++) {
            ...
            $this->excel->getActiveSheet()->getStyle('A2')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB("ffe6b8");
            $this->excel->getActiveSheet()->getStyle('C2')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB("ffe6b8");
            $this->excel->getActiveSheet()->getStyle('E2')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB("ffe6b8");
            $this->excel->getActiveSheet()->getStyle('G2')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB("ffe6b8");
