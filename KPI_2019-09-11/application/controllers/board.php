...
class Board extends CI_Controller {
	...
	********************************/
		public function __construct()
		{
			...
			$this->pagination_config['base_url'] = site_url('/bbs/board/lists/'.$this->category.'');
	...
	********************************/
		public function retrieve()
		{
			...
			if ($this->boardm->get_article($model_data) == FALSE) {
				redirect('/bbs/board/lists/'.$this->category.'/');
			}
	...
	********************************/
		public function update()
		{
			...
			if ($user_data['uid'] == $this->view_data['retrieve']['writer']) {
				...
				redirect('/bbs/board/retrieve/'.$this->category.'/'.$this->uri->segment(4,0).'/'.$model_data['uid']);
			}
			...
			if ($form_validate == FALSE) {
				...
				if ($this->boardm->article_update($model_data)) {
					redirect('/bbs/board/retrieve/'.$this->category.'/'.$this->uri->segment(4,0).'/'.$this->boardm->last_article());
				}
			}
		}
	...
	********************************/
		public function erase()
		{
			...
			redirect('/bbs/board/lists/'.$this->category.'/'.$this->uri->segment(4,0).'/');
		}
		...
		public function comment_write()
		{
			...
			if ($this->input->post('comment_mode') == 'create' && $this->input->post('comment_uid') == '0') {
				...
				if ($this->boardm->comment_write($model_data)) {
					...
					redirect('/bbs/board/retrieve/'.$this->category.'/'.$this->uri->segment(4,0).'/'.$this->uri->segment(5,0));
				}
			}
