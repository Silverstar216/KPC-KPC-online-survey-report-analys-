...
class D_question_groups_model extends MY_Model
{
    public function __construct()
    {
        parent::__construct();
        $this->table_name = 'd_question_groups';
    }
    public function getExampleData($survey_id){
        ...
        foreach ($data as $question_group) {
            ...
            $sql="";
            $sql.="select * from d_questions where question_group_id=" . $question_group['id'];
            ...
            foreach($result_question_group as $question) {
                ...
                if($question['type'] == 3){ //강사만족도일때
                    ...
                    //평가지표얻기
                    $sql .= "select * from d_question_exam_kinds where question_id=" . $question['id'];
                    $query = $this->db->query($sql);
                    $question['exam_kinds'] = $query->result_array();

                    //평가대상얻기
                    $sql = "";
                    $sql .= "select * from d_question_exam_objects where question_id=" . $question['id'];
                    $query = $this->db->query($sql);
                    $question['exam_objects'] = $query->result_array();
                }else{                      //Not 강사만족도

                    $sql .= "select * from d_examples where question_id=" . $question['id'];
                    $query = $this->db->query($sql);
    ...
    public function delete_by_id($id)
    {
        ...
        $sql = "select id from d_questions where survey_id =".$id;
        $query = $this->db->query($sql);
        $questions =$query->result_array();
        foreach ($questions as $index) {
            $sql = "delete from d_examples where question_id=".$index['id'];
            $query = $this->db->query($sql);
        }
